...
import vm.VirtualMachine.LoadImIntExec;

public class SwitchCmd extends AbsSynTreeNode implements ICmd {
	private IExpr expr;
	private ArrayList<Case> cases;
	private DefaultCase defaultCase;
	
	public SwitchCmd(IExpr expr, ArrayList<Case> cases, DefaultCase defaultCase) {
		this.expr = expr;
		this.cases = cases;
	...
	public void saveNamespaceInfoToNode(HashMap<String, TypedIdent> localStoresNamespace)
		...
		for(Case c : cases)
			c.saveNamespaceInfoToNode(DeepCopyHelper.deepCopy(this.localStoresNamespace));
		
		// add deepCopy from local storage to the default case
		defaultCase.saveNamespaceInfoToNode(DeepCopyHelper.deepCopy(this.localStoresNamespace));
	}
	...
	public void doScopeChecking() throws NameNotDeclaredError, LRValError, InvalidParamCountError, CaseAlreadyDeclaredError, DefaultCaseBoolOverkillError {
		...
		for(Case c1 : cases) {
			for(Case c2 : cases) {
				if(c1 != c2) {
					if(c1.getLiteral().getType() == Type.INT64) {
						if(c1.getLiteral().getIntValue() == c2.getLiteral().getIntValue())
							throw new CaseAlreadyDeclaredError("" + c1.getLiteral().getIntValue());
					} else if (c1.getLiteral().getType() == Type.BOOL) {
						if(c1.getLiteral().getBoolValue() == c2.getLiteral().getBoolValue())
							throw new CaseAlreadyDeclaredError("" + c1.getLiteral().getBoolValue());					
					}
				}
			}
		}
		...
		if(cases.size() == 2 && cases.get(0).getLiteral().getType() == Type.BOOL && defaultCase != null)
...
//			if(!simulateOnly)
//				codeArray.put(codeArrayPointer, new IInstructions.LoadImInt(cases.get(i).getLiteral().getIntValue()));
//			codeArrayPointer++;
...
//	}
	...
	public void addIInstrToCodeArray(HashMap<String, Integer> localLocations, boolean simulateOnly)
		...
		if(cases.size() > 0 && cases.get(0).getLiteral().getType() == Type.INT64) {
			// find the case with the smallest and largest case literal
			minCaseLiteral = cases.get(0).getLiteral().getIntValue();
			maxCaseLiteral = cases.get(0).getLiteral().getIntValue();
			for(int i = 1; i < cases.size(); i++) {
				int caseLiteral = cases.get(i).getLiteral().getIntValue(); 
		...
		// Type is boolean
		} else if (cases.size() > 0 && cases.get(0).getLiteral().getType() == Type.BOOL) {
			minCaseLiteral = cases.get(0).getLiteral().getBoolValue() ? 1 : 0;
			maxCaseLiteral = cases.get(0).getLiteral().getBoolValue() ? 1 : 0;
			int caseLiteral = cases.size() > 1 ? (cases.get(1).getLiteral().getBoolValue() ? 1 : 0) : 0; 
			if(caseLiteral < minCaseLiteral)
				minCaseLiteral = caseLiteral;
			if(caseLiteral > maxCaseLiteral)
				maxCaseLiteral = caseLiteral;
		...
		for(int i = 0; i < cases.size(); i++) {
			if(cases.get(i).getLiteral().getType() == Type.INT64) {
				jumpTableAddresses[cases.get(i).getLiteral().getIntValue() - caseLiteralOffset] = firstCasePosition + caseRelPositions[i];
			} else if (cases.get(i).getLiteral().getType() == Type.BOOL) {
				if(cases.get(i).getLiteral().getBoolValue() == true) {
